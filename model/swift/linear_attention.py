...
class Attention(Module):
    ...
    def export(self, query, key, value):
        ...
        # # return m
        # Remove einsums for model export
        # # QK = torch.einsum("nlhd,nshd->nlsh", query, key)
        n,l, h, d = query.shape
        _,s, _, _ = key.shape
        qt= query.permute((0,2,1,3)).reshape(n*h,l,d)
        kt= key.permute((0,2,3,1)).reshape(n*h,d,s)

        # qt= query.permute((0,2,1,3)).view(n*h,l,d)
        # kt= key.permute((0,2,3,1)).view(n*h,d,s)
        QK = torch.bmm(qt, kt) # n*h, l, s
        # n,h,l,s to nlsh
        QK = QK.reshape(n,h,l,s).permute((0,2,3,1))
        ...
        # Remove einsums for model export
        # # out = torch.einsum("nlsh,nshd->nlhd", A, value)
        # n,l, s, h -> n,h,l,s -> n*h, l, s
        # at= A.permute((0,3,1,2)).view(n*h,l,s)
        at= A.permute((0,3,1,2)).reshape(n*h,l,s)


        # n,s, h, d -> n,h, d, s -> n*h, s,d
        # vt= value.permute(((0,2,1,3))).view(n*h,s,d)
        vt= value.permute(((0,2,1,3))).reshape(n*h,s,d)
        out = torch.bmm(at, vt) # n*h, l, d
        out = out.reshape(n,h,l,d)
        # n,h,l,d to nlhd
        out = out.permute((0,2,1,3))
    
        return out

    def forward2(self, query, key, value, q_mask=None, kv_mask=None):
        return self.export(query, key, value)
