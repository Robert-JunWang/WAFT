...
from model.backbone.dinov3 import DinoV3Feature2
...
class SWIFT(nn.Module):
    def __init__(self, args):
        ...
        if args.feature_encoder == 'twins':
            ...
            self.encoder = DinoV3Feature2(model_name="vits", output_dim=256,lvl=-3)
            self.factor = 16
        else:
        ...
        self.coarse_matcher = CoarseMatcher(self.pretrain_dim)
        self.iter_dim = MODEL_CONFIGS[args.iterative_module]['features']
        self.fnet = ResNet18Deconv(3, self.iter_dim)
        self.refine_net = VisionTransformer(args.iterative_module, self.iter_dim, patch_size=8)
        self.fmap_conv = nn.Conv2d(self.iter_dim*2, self.iter_dim, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def preprocessing(self, image1, image2):
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        flow_predictions, info_predictions = self._forward(image1, image2, iters)
    ...
    def _forward(self, image1, image2, iters):
        ...
        # flow_2x = self.coarse_matcher(fmap1_pretrain, fmap2_pretrain)

        x = torch.cat([image1, image2], dim=0)
        fmap_pretrain = self.encoder(x)
        # print('fmap_pretrain', fmap_pretrain.shape)

        flow_2x = self.coarse_matcher(fmap_pretrain)
        # print('coarse matching', flow_2x.shape)
        # x = torch.cat([image1, image2], dim=0)
        fmap = self.fnet(x)[0]
        # print('fmap', fmap.shape)
        fmap1_2x, fmap2_2x = fmap.chunk(2)

        fmap1_2x = self.fnet(image1)[0]
        fmap2_2x = self.fnet(image2)[0]
    ...
    def export2(self, image1, image2, iters=None):
        """ Estimate optical flow between pair of frames """
        if iters is None:
            iters = self.args.iters
        flow_predictions, _ = self._forward(image1, image2, iters)
        
        return flow_predictions[-1]

    def export(self, image1, image2, iters=1):
